...
wait_elastic() {
    ...
    while [ $elastic -eq 0 ]
    do
        curl -s "http://localhost:9400" 2>&1 > /dev/null \
            && elastic=1 \
...
#         
#     unset version
    #wget -q https://download.elastic.co/elasticsearch/release/org/elasticsearch/distribution/deb/elasticsearch/2.4.0/elasticsearch-2.4.0.deb -O /tmp/elasticsearch.deb
...
curl -s -XPUT 'http://localhost:9400/_snapshot/backups' \
...
# Add the bitbucket's host keys to known hosts 
...
CELERY_HOST=${CELERY_HOST:-'localhost'}
...
REDIS_URL=${REDIS_URL:-redis://$CELERY_HOST:6379/1}
...
#cat <<EOF > /opt/superdesk/client/dist/config.js
#window.superdeskConfig={
#    apps: ['superdesk-publisher'],
#    publisher: {
#        protocol: "http",
#        tenant: '', // subdomain
#        domain: '192.168.13.6', // IP address or domain name of your server where Superdesk Publisher is installed
#        base: 'api/v1'
#    },
#};
#EOF
